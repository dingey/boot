...
public class PermissionController {
    ...
    @Autowired
    public PermissionController(PermissionService permissionService, RequestMappingHandlerMapping requestMappingHandlerMapping) {
    ...
    @ResponseBody
    @PostMapping(path = "/admin/permission/sorts")
    public String sort(String ids) {
        if (StringUtils.hasText(ids)) {
            String[] split = ids.split(",");
            for (int i = 0; i < split.length; i++) {
                permissionService.updateSequence(Integer.valueOf(split[i]), i);
            }
        }
        return "success";
    }
